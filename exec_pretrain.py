...
def train(logger):
    ...
    # TODO: 加载预先训练模型
    pre_train_path=os.path.join(cf.fold_dir,"pretrain_model")
    pre_model = utils.import_module('model', os.path.join(pre_train_path,"model.py"))
    cf.operate_stride1=False #not add P0
    pre_net = pre_model.net(cf, logger).cuda()
    cf.operate_stride1 = True
    pre_model_path=os.path.join(os.path.join(cf.fold_dir,"pretrain_model"), "249_best_checkpoint")
    checkpoint = torch.load(os.path.join(pre_model_path, 'params.pth'))
    pre_net.load_state_dict(checkpoint)
    pre_net_dict=pre_net.state_dict()

    net_dict=net.state_dict()
    pretrained_dict={k:v for k,v in pre_net_dict.items() if k in net_dict and k!="Fpn.C1.0.weight"}
