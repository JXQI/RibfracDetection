...
def get_train_generators(cf, logger):
    ...
    # train_pids=train_pids[:10]
    # val_pids=val_pids[:10]
    print("train_len={},val_len={}".format(len(train_pids),len(val_pids)))
...
def get_test_generator(cf, logger):
    ...
    logger.info("data set loaded with: {} val patients".format(len(test_ix)))
    batch_gen = {}
...
def load_dataset(cf, logger, subset_ixs=None, pp_data_path=None, pp_name=None):
    ...
    imgs = [os.path.join(pp_data_path, '{}_img.npz'.format(pid)) for pid in pids]
    # segs = [os.path.join(pp_data_path,'{}_rois.npz'.format(pid)) for pid in pids]
    # imgs_npy=[]
    # segs_npy=[]
    # for i,_data in enumerate(imgs):
    #     data_npz=np.load(_data, mmap_mode='r')
    #     img_arr=data_npz["img"].astype(np.float32)
    #     roi_arr=data_npz["rois"]
    #     img_arr = (img_arr - np.mean(img_arr)) / np.std(img_arr).astype(np.float16)
    #     imgs_npy.append(np.transpose(img_arr, axes=(1, 2, 0))[np.newaxis])
    #     segs_npy.append(np.transpose(roi_arr, axes=(1, 2, 0)))
    data = OrderedDict()
    for ix, pid in enumerate(pids):
        ...
        # data[pid] = {'data': imgs_npy[ix], 'seg': segs_npy[ix], 'pid': pid, 'class_target': targets}
        data[pid] = {'data': imgs[ix],  'pid': pid, 'class_target': targets}
...
def create_data_gen_pipeline(patient_data, cf, is_training=True):
    ...
    multithreaded_generator = MultiThreadedAugmenter(data_gen, all_transforms, pin_memory=True,num_processes=cf.n_workers, seeds=range(cf.n_workers))
    return multithreaded_generator
...
class BatchGenerator(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        for index,b in enumerate(batch_ixs):
            start_batch = time.time()
            patient = patients[b][1]

            # data shape: from (z,y,x) to (c, y, x, z).
            # data = np.transpose(np.load(patient['data'], mmap_mode='r')["img"], axes=(1, 2, 0))[np.newaxis]
            # seg = np.transpose(np.load(patient['seg'], mmap_mode='r')["img"], axes=(1, 2, 0))
            # data=patient['data']
            # seg=patient['seg']
            data = patient['data']
            data_npz = np.load(data, mmap_mode='r')
            ...
            load_time=time.time()
            # print("load_time={}".format(time.time()-start_batch))
            if self.cf.dim == 2:
            ...
            if len(crop_dims) > 0:
                ...
                crop_end=time.time()
                # print("index={},crop_time={},fg_prob={}".format(index,crop_end-crop_begin,fg_prob_sample))
                # print(data.shape,crop_dims)
                for ii in crop_dims:
                ...
                crop_take=time.time()
                # print("index={},crop_take={}".format(index,crop_take-crop_end))
            batch_data.append(data)
            batch_segs.append(seg[np.newaxis])
            stop=time.time()
            # print("{} patch spent time ={}".format(index,stop-load_time))
        # print("\nbatch size spent time={}".format(stop-start))
...
#test_patients
def get_test_generator_test(cf, logger,data_set):
    """
    wrapper function for creating the test batch generator pipeline.
    selects patients according to cv folds (generated by first run/fold of experiment)
    If cf.hold_out_test_set is True, gets the data from an external folder instead.
    ...
    if cf.hold_out_test_set:
        ...
        # train
            ...
            train_ix,_,test_ix, _ = fold_list[cf.fold]
    ...
    batch_gen = {}
    batch_gen['test'] = PatientBatchIterator_test(test_data, cf=cf)
    batch_gen['n_test'] = len(test_ix) if cf.max_test_patients=="all" else \
        min(cf.max_test_patients, len(test_ix))
    return batch_gen

class PatientBatchIterator_test(SlimDataLoaderBase):
    """
    creates a test generator that iterates over entire given dataset returning 1 patient per batch.
    Can be used for monitoring if cf.val_mode = 'patient_val' for a monitoring closer to actualy evaluation (done in 3D),
    if willing to accept speed-loss during training.
    ...
    """
    def __init__(self, data, cf): #threads in augmenter
        super(PatientBatchIterator_test, self).__init__(data, 0)
        self.cf = cf
        self.patient_ix = 0
        self.dataset_pids = [v['pid'] for (k, v) in data.items()]
        self.patch_size = cf.patch_size
    ...
    def generate_train_batch(self,index):
        pid = self.dataset_pids[index]
        print(index,pid)
        # self.patient_ix=1
        # print(self.patient_ix)
        patient = self._data[pid]
        data = patient['data']
        data_npz = np.load(data, mmap_mode='r')
        data = data_npz["img"].astype(np.float32)
...
if __name__=="__main__":
    ...
    cf.exp_dir = "/Users/jinxiaoqiang/jinxiaoqiang/DATA/Bone/ribfrac/dataloader_test"
    ...
    print(test_batch['seg'].shape)
    print(test_batch['original_img_shape'])
    print(test_batch['class_target'].shape)
