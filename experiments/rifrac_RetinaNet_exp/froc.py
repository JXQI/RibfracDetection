...
def froc_evaluate(det_results,dst_path):
    ...
    with open(os.path.join(dst_path, 'froc.txt'), 'a') as handle:
        handle.write("\n the froc of this detection \n")
        handle.write(str(pd.DataFrame(np.array(eval_results["detection"]["key_recall"])\
        .reshape(1, -1), index=["Recall"],columns=[f"FP={str(x)}" for x in DEFAULT_KEY_FP])))
        handle.write("\nAverage recall: {:.4f}\n".format(eval_results["detection"]["average_recall"]))
        handle.write("Maximum recall: {:.4f}\n".format(eval_results["detection"]["max_recall"]))
        handle.write("Average FP per scan at maximum recall: {:.4f}\n".format(eval_results["detection"]["average_fp_at_max_recall"]))
# add to evaluator
...
if __name__ == "__main__":
    ...
    test_df="./examples/399/fold_0_test_df.pickle"
