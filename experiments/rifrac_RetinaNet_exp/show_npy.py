...
class test_result:
    ...
    def read_batchData_pickle(self,scores=0.1):
        ...
        data = np.load(os.path.join(self.path, '{}_img.npz'.format(pid)))["img"]
        data_nii = sitk.GetImageFromArray(data)
        sitk.WriteImage(data_nii, os.path.join(dstpath, "{}_image.nii.gz".format(pid)))
        # deal with raw_result
        print("deal raw_result:")
    ...
    def compute_roi_ap(self,df, all_p):
        ...
        if True:
            plt.figure()
            plt.plot(rc,pr)
            plt.savefig("./examples/PR_curve.png")
        is_froc=False
        if is_froc:
            plt.figure()
            fps=fp_sum/1
            ...
            plt.ylabel("recall")
            plt.xlabel("FP/all_imgs")
            plt.savefig("./examples/froc_curve.png")
        return np.mean(q)
...
if __name__=="__main__":
    ...
    path="/media/victoria/9c3e912e-22e1-476a-ad55-181dbde9d785/jinxiaoqiang/medicaldetectiontoolkit/experiments/rifrac_RetinaNet_test/fold_ids.pickle"
    df=pd.read_pickle(path)
    print(df)