...
class Predictor:
    ...
    def __init__(self, cf, net, logger, mode):
        ...
        self.rank_ix = 0
        self.weight_paths = [os.path.join(self.cf.fold_dir, '{}_best_checkpoint'.format(epoch), 'params.pth') for
                             epoch
                             in self.epoch_ranking]
        # self.logger.info(
        #     ('tmp ensembling over rank_ix:{} epoch:{}'.format(self.rank_ix, self.weight_paths[self.rank_ix])))
        self.net.load_state_dict(torch.load(self.weight_paths[self.rank_ix]))
    ...
    # test all patient
    def predict_test_set_all(self,batch_gen,pid,result_dir, return_results=True):
        """
        ...
        with torch.no_grad():
            ...
            batch=batch_gen.generate_train_batch(pid)
        ...
        out_string = results_per_patient[-1][-1]+"_"+out_string
        # path=os.path.join(self.cf.fold_dir,self.mode)
        path=os.path.join(result_dir,self.mode)
        if not os.path.exists(path):
            os.mkdir(path)
        with open(os.path.join(path, '{}.pickle'.format(out_string)), 'wb') as handle:
